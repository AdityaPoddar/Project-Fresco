<?php
        ...
        $query_user="SELECT DISTINCT SHOP_ID FROM SHOP WHERE FK1_USER_ID='$userid'";
        // echo $query_user;
        // exit();
        $shop_result=oci_parse($connection,$query_user);
        ...
        $query="SELECT ORDERDETAIL_ID, ORDERDETAIL_QUANTITY, FK1_ORDER_ID, FK2_PRODUCT_ID, od.ORDER_DATE, FK3_SHOP_ID 
        FROM ORDER_DETAIL od, ORDERS os 
        WHERE os.ORDER_ID = od.FK1_ORDER_ID AND FK3_SHOP_ID = '$shopid'";
        ...
        <section class="dashboard">
            ...
            <div class="bottom-section">
                <div class="button">
                        ...
                        <!-- <form action="traderorder.php" method="POST"> -->
             <?php            
             while ($row=oci_fetch_assoc($shop_result))  {
                 $shopid=$row['SHOP_ID'];
                ...
                <a href='traderorder.php?sid=$shopid'>
                <button name='submit' id='ordershop1' class='btn'>SHOP-$shopid</button>
                           ...
                           if(isset($_GET['sid'])) {
                            ...
                            {
                            $orderdetailid= $value['ORDERDETAIL_ID'];
                            $orderid= $value['FK1_ORDER_ID'];
                            $productid= $value['FK2_PRODUCT_ID'];
                            $orderdate= $value['ORDER_DATE'];
                            $shopid1= $value['FK3_SHOP_ID'];
