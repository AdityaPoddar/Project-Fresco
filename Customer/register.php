<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $user_check_query = "SELECT * FROM USERS WHERE USER_NAME = '$username' OR USER_EMAIL = '$email'";
        $result = oci_parse($conn, $user_check_query);
        oci_execute($result);
        ...
        if (($empty_username || $empty_email || $empty_password || $nomatch || $username_exists || $email_exists) != 1) {
            $user_password = md5($password);
            $query = "INSERT INTO USERS (USER_NAME, USER_EMAIL, PASSWORD, USER_TYPE, OTP) VALUES('$username', '$email', '$user_password','Customer', 0)";
            $insert_query = oci_parse($conn, $query);
            $check = oci_execute($insert_query);
        ...
        <div class="log-section">
            <div class="info-card">
                <div class="info-container">
                    ...
                    <h5><a href="trader/signup.php">Become a trader</a></h5>
                    <h5>Already a customer?</h5>
                    <a href="login.php"><button id="login-btn">Sign In</button></a>
                </div>
            </div>
            <div class="log-card">
                <form action="register.php" method="post">
                    ...
                    <input type="submit" name="submit" value="Sign Up" id="register-btn">
                </form>
            </div>
        </div>
    </div>
